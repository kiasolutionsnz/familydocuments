...
| PB-03 | Telegram transport completion | In progress — staging | Private-user linking, follow-ups and attachment handling use the shared orchestrator. Staging now exposes the Settings flow and a truthful setup-incomplete state, while keeping connection and webhook routes closed until the bot is configured. The supervised worker launch path and isolated-gateway rollback are prepared. Staging has no Telegram bot identity, username or webhook secret configured, so no real Telegram traffic is enabled. The dedicated `@FamilyDocumentsapp_bot`, server-side secrets and explicit approval for webhook registration are required before end-to-end acceptance. Family-group scope remains excluded. |
