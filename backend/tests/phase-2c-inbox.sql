...
begin
  ...
  insert into fp.inbound_emails(id,household_id,inbox_alias_id,source_system,external_message_id,sender_address,recipient_addresses,subject,raw_email,raw_sha256,raw_size_bytes,body_text,attachment_manifest,attachment_count,attachment_status,processing_status,sender_disposition,ingested_at) values
    (message_one,family_id,alias_id,'mailpit_local','inbox-one','sender@example.test','["family@example.test"]','Insurance invoice',pdf,repeat('1',64),octet_length(pdf),'<script>alert(1)</script><img src="https://tracker.example/pixel">Please review https://example.com/policy','[]',1,'quarantined_unscanned','needs_review','allowed',now()),
  ...
  insert into fp.inbound_attachments(id,inbound_email_id,household_id,mailpit_part_id,file_name,claimed_content_type,expected_size_bytes,expected_sha256,content,content_sha256,scan_status)
    values(attachment_id,message_one,family_id,'part-1','synthetic-invoice.pdf','application/pdf',octet_length(pdf),encode(extensions.digest(pdf,'sha256'),'hex'),pdf,encode(extensions.digest(pdf,'sha256'),'hex'),'clean');
  ...
  begin perform fp.inbox_message_detail(message_one);raise exception 'revoked member retained access';exception when sqlstate 'PT404' then null;end;
end$$;
