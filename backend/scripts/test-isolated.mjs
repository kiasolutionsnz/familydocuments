...
try {
  if (manualPhase2F) {
    ...
    const driveNames = ['GOOGLE_DRIVE_CLIENT_ID', 'GOOGLE_DRIVE_CLIENT_SECRET', 'GOOGLE_DRIVE_TOKEN_KEY'];
    if (driveNames.some(name => process.env[name]) && driveNames.some(name => !process.env[name])) throw new Error('Phase 2F Drive gateway settings are incomplete');
    if (process.env.FD_STAGING_ORIGIN && !/^https:\/\/[a-z0-9.-]+$/.test(process.env.FD_STAGING_ORIGIN)) throw new Error('Invalid staging origin');
  }
