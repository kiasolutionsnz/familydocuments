...
language sql stable security definer set search_path=pg_catalog,fp as $$
  select fp.is_active_member(reminder_row.household_id) and (
    reminder_row.created_by=fp.current_user_id() or fp.is_household_admin(reminder_row.household_id) or
    (reminder_row.document_id is not null and exists(
...
language plpgsql security definer set search_path=pg_catalog,fp as $$
declare uid uuid:=fp.current_user_id();r fp.reminders;next_due date;next_id uuid;
begin
...
language plpgsql security definer set search_path=pg_catalog,fp as $$
declare uid uuid:=fp.current_user_id();hid uuid;member_role text;row fp.reminders;
begin
...
create table if not exists fp.document_analysis_jobs(
  ...
  status text not null default 'queued' check(status in ('queued','processing','retry_wait','succeeded','failed','permanent_failed')),
...
declare uid uuid:=fp.current_user_id();j fp.document_analysis_jobs;
begin
  update fp.document_analysis_jobs x set status='queued',attempts=0,next_attempt_at=now(),lease_expires_at=null,
...
begin
  ...
  update fp.document_analysis_jobs j set status='permanent_failed',failure_code='source_unavailable',completed_at=now(),lease_expires_at=null,updated_at=now()
  ...
  with leased as(
    ...
    update fp.document_analysis_jobs j set status='processing',attempts=j.attempts+1,started_at=coalesce(j.started_at,now()),
      lease_expires_at=now()+interval '5 minutes',updated_at=now() from leased where j.id=leased.id returning j.*
...
create or replace function fp.complete_document_analysis_job(job uuid,job_result jsonb) returns jsonb
language plpgsql security definer set search_path=pg_catalog,fp as $$
declare j fp.document_analysis_jobs;selected_category_id uuid;title_value text;tags_value jsonb;document_date_value date;
begin
  if coalesce(nullif(current_setting('request.jwt.claim.role',true),''),nullif(current_setting('request.jwt.claims',true),'')::jsonb->>'role','')<>'service_role' then raise exception 'not authorised' using errcode='42501';end if;
  select * into j from fp.document_analysis_jobs where id=job and status='processing' and lease_expires_at>now() for update;
  ...
  update fp.document_analysis_jobs set status='succeeded',result=job_result-'text',failure_code=null,lease_expires_at=null,completed_at=now(),updated_at=now() where id=j.id;
  return jsonb_build_object('job_id',j.id,'document_id',j.document_id,'status','succeeded');
end $$;

create or replace function fp.fail_document_analysis_job(job uuid,error_code text,retryable boolean default true) returns jsonb
language plpgsql security definer set search_path=pg_catalog,fp as $$
declare j fp.document_analysis_jobs;next_status text;
begin
  if coalesce(nullif(current_setting('request.jwt.claim.role',true),''),nullif(current_setting('request.jwt.claims',true),'')::jsonb->>'role','')<>'service_role' then raise exception 'not authorised' using errcode='42501';end if;
  select * into j from fp.document_analysis_jobs where id=job and status='processing' for update;
  if j.id is null then raise exception 'job unavailable' using errcode='PT404';end if;
  ...
  update fp.document_analysis_jobs set status=next_status,failure_code=left(coalesce(nullif(error_code,''),'processing_failed'),80),
    lease_expires_at=null,next_attempt_at=case when next_status='retry_wait' then now()+(power(2,j.attempts)::text||' minutes')::interval else next_attempt_at end,
...
begin
  ...
  return jsonb_build_object(
    ...
    'notifications',coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'title',r.title,'due_at',r.due_at,'due_time',r.due_time,'due_time_zone',r.due_time_zone,'due_state',case when r.due_at<current_date then 'overdue' when r.due_at=current_date then 'today' else 'due_soon' end,'audience',r.audience,'my_response',x.status) order by r.due_at,r.due_time nulls last)
      ...
      where r.household_id=hid and r.status='upcoming' and r.due_at<=current_date+7 and (r.document_id is null or d.lifecycle_status='active') and ((r.audience='personal' and r.created_by=uid) or r.audience='family')),'[]'::jsonb),
    'counts',jsonb_build_object(
      'overdue',(select count(*) from fp.reminders r left join fp.documents d on d.id=r.document_id where r.household_id=hid and r.status='upcoming' and r.due_at<current_date and (r.document_id is null or d.lifecycle_status='active') and ((r.audience='personal' and r.created_by=uid) or r.audience='family')),
      'due_soon',(select count(*) from fp.reminders r left join fp.documents d on d.id=r.document_id where r.household_id=hid and r.status='upcoming' and r.due_at between current_date and current_date+7 and (r.document_id is null or d.lifecycle_status='active') and ((r.audience='personal' and r.created_by=uid) or r.audience='family')),
      'unread',(select count(*) from fp.reminders r left join fp.reminder_member_responses x on x.reminder_id=r.id and x.member_user_id=uid left join fp.documents d on d.id=r.document_id where r.household_id=hid and r.status='upcoming' and r.due_at<=current_date+7 and (r.document_id is null or d.lifecycle_status='active') and ((r.audience='personal' and r.created_by=uid) or (r.audience='family' and coalesce(x.status,'pending')='pending')))
    )
  );
end $$;
...
revoke execute on function fp.create_reminder(text,date,time,text,uuid,text),fp.create_document_analysis_job(text,text,text,text,text),
  ...
  fp.categorize_document_analysis_job(uuid,text),
  fp.claim_document_analysis_jobs(integer),fp.complete_document_analysis_job(uuid,jsonb),fp.fail_document_analysis_job(uuid,text,boolean)
  from public,anon;
grant execute on function fp.create_reminder(text,date,time,text,uuid,text),fp.create_document_analysis_job(text,text,text,text,text),
  fp.document_analysis_job(uuid),fp.pending_document_analysis_jobs(),fp.retry_document_analysis_job(uuid),fp.categorize_document_analysis_job(uuid,text) to authenticated;
revoke execute on function fp.claim_document_analysis_jobs(integer),fp.complete_document_analysis_job(uuid,jsonb),fp.fail_document_analysis_job(uuid,text,boolean) from authenticated;
grant execute on function fp.claim_document_analysis_jobs(integer),fp.complete_document_analysis_job(uuid,jsonb),fp.fail_document_analysis_job(uuid,text,boolean) to service_role;
