...
test("ships isolated local auth, invitation, access and category boundaries", async () => {
  ...
  assert.match(data, /google_drive_connection_summary/);
  ...
  assert.match(drive, /setMaxItems\(1\)/);
  assert.match(drive, /ViewId\.FOLDERS/);
  assert.match(drive, /pickFolder/);
  assert.match(drive, /Google did not return a valid folder/);
  assert.match(drive, /createFolder/);
  assert.match(drive, /application\/vnd\.google-apps\.folder/);
  assert.match(drive, /uploadType=media/);
  assert.match(drive, /setOrigin\(location\.origin\)/);
  assert.match(drive, /alt=media/);
  assert.match(drive, /15\*1024\*1024/);
  ...
  assert.match(app, /Check linked files/);
