...
const groups=[
  ...
  {id:"privacy",title:"Privacy and sharing",items:[
    ["Are records shared with everyone by default?","No. Records and saved links are private by default. You choose what to share and which family members can see it."],
  ...
  {id:"support",title:"Support",items:[["What should I do when something is not working?","Record the page name and exact error text, then email support@familydocuments.app. Never send a password, verification code, access token, or secret key."]]},
] as const;
...
export default function FAQ(){return <><nav className="site-nav content-nav"><a className="wordmark" href="/"><span>FD</span><b>Family Documents</b></a><div className="nav-links"><a href="/blog">Blog</a><a className="nav-auth nav-sign-in" href="/prototype/index.html#auth">Sign in</a><a className="button small nav-auth" href="/prototype/index.html#auth">Register free</a></div></nav><main className="content-page"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema).replace(/</g,"\\u003c")}}/><section className="content-hero"><p className="eyebrow">Frequently asked questions</p><h1>Clear answers about Family Documents.</h1><p>Find help with setup, storage, documents, OCR, forwarded email, reminders, privacy, travel, rentals, and saved links. Or use “Ask us” for a guided answer from this same approved help content.</p></section><div className="faq-list">{groups.map(group=><section className="faq-group" id={group.id} key={group.id}><h2>{group.title}</h2>{group.items.map(item=><details key={item[0]}><summary>{item[0]}</summary><p>{item[1]}</p></details>)}</section>)}</div></main></>}
