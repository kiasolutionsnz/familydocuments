(function attachFamilyPassportData(global) {
  ...
  global.familyPassportData = Object.freeze({
    ...
    createOcrIntakeDraft: value => rpc("create_ocr_intake_draft", value),
    recordOcrIntakeResult: value => rpc("record_ocr_intake_result", value),
    confirmOcrIntake: value => rpc("confirm_ocr_intake", value),
    rejectOcrIntake: draft => rpc("reject_ocr_intake", {draft}),
    ...
    inboundEmailDetail: email => rpc("inbound_email_detail", {email}),
    inboundSenderRules: () => rpc("inbound_sender_rule_summaries"),
    setInboundSenderRule: (sender, action) => rpc("set_inbound_sender_rule", {sender, rule_action:action}),
    moveInboundEmailToBin: (email, restore=false) => rpc("move_inbound_email_to_bin", {email, restore}),
